...
export default function AddCalorieForm(props){
    ...
    const handleAddClick = async (event) => {
        ...
        try {
            ...
            if(calorie === '' || description === '' || !Number.isInteger(calorie)){
                throw Error;
            }
        }
        ...
        console.log('curr DEBUG: ' + document.getElementById('month-filter').value);

        // After adding new item, we want the list will be refreshed.
        if (document.getElementById('month-filter').value){
            props.filterFunc(document.getElementById('month-filter').value);
        }
        else{
