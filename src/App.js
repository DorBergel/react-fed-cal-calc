...
import _CaloriesList from "./_CaloriesList";
...
function App() {
    ...
    const mappingItems = function(items) {
        return (
            items.map((item) => {
                ...
                return (
                    <Card variant="outlined" sx={{ width: "98%", backgroundColor: "#212529" }} className={'cal-ticket'} key={item.id}>
                        <Box sx={{ p: 2, display: 'flex', alignItems: 'center' }}>
                            ...
                            <Typography sx={{whiteSpace:"nowrap", marginLeft: "5px"}} gutterBottom variant="h6" component="div">
                                {item.calorie} Cal.
                            </Typography>
                        </Box>
                        <Divider />
    ...
    const handleFilterChange = async (filterValue) => {
        ...
        // The process of showing records of specific month and year.
            ...
            const [selectedYear, selectedMonth] = filterValue.split('-').map(Number);
